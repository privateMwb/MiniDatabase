...
`packaging/recipes/minidb/all/conanfile.py`). Scoped to what isn't obvious
...
- **The release tarball extracts to `MiniDatabase-<version>`, not
  `MiniDB-<version>`** — the GitHub repo is `MiniDatabase`; the CMake
  project/library target inside it is `MiniDB`. `cd` into whatever
...
```bash
find $HOME/staging/lib/cmake/MiniDB -name "*.cmake"
```

Expect `MiniDBConfig.cmake`, `MiniDBConfigVersion.cmake`,
`MiniDBTargets.cmake` (plus a per-config
`MiniDBTargets-<config>.cmake`) under `lib/cmake/MiniDB/`.
...
target_link_libraries(test PRIVATE MiniDB::MiniDB)
EOF
...
int main() {
    Database db("test");
    if (db.createTable("t", Vector<ColumnDef>{ColumnDef{"v", ColumnType::INT, false}}) != Status::OK) return 1;
    Table* t = db.getTable("t");
    Record r(1);
    (void)r.setField("v", Json(42));
    if (t->insertRecord(r) != Status::OK) return 1;
    Record out;
    if (t->getRecord(1, out) != Status::OK || out.getField("v").asNumber() != 42) return 1;
    return 0;
}
EOF
...
creates a database and table, inserts a record, and reads it back, so
success confirms the header parsed, the library linked, the symbols
resolved, and a real operation actually works end to end.
...
```bash
conan create packaging/recipes/minidb/all --version 1.0.0 \
...
`packaging/recipes/minidb/all/test_package/` against the freshly-built
package — a `MiniDB linked and constructed successfully.` line in
