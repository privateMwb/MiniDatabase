...
// border/column to its right out of alignment. Walks codepoints (not
// bytes) so a multi-byte character never gets cut in half. Mirrors
// the regression tool's own truncateCell(), just built on this file's
// visualWidth() instead of displayWidth().
...
inline void setHeader(std::string_view header) {
    ...
    std::cout << GRAY << V << RESET
              << BOLD << CYAN << padCell(" " + truncateCell(prettify(header), TEST_W - 1), TEST_W) << RESET
              << GRAY << V << RESET
...
inline void printSoloRow(std::string_view name, std::string_view iteration, nanoseconds ns) {
    ...
    // clang-format off
    std::cout << GRAY << V << RESET
              << padCell(" " + truncateCell(prettify(name), TEST_W - 1), TEST_W)
              << GRAY << V << RESET
