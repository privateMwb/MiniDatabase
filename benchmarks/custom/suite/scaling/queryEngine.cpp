...
static void bench_select_eq_at_scale() {
    ...
    BENCH_SOLO_CUSTOM("qe select eq(1)", selSmall);

    auto selMedium = [&] {
        QueryResult r = engine.select(medium, preds);
        doNotOptimize(r);
    };
    BENCH_SOLO_CUSTOM("qe select eq(100)", selMedium);

    auto selLarge = [&] {
        QueryResult r = engine.select(large, preds);
        doNotOptimize(r);
    };
    BENCH_SOLO_CUSTOM("qe select eq(1000)", selLarge);
}
...
static void bench_select_sort_at_scale() {
    ...
    BENCH_SOLO_CUSTOM("qe select sorted(1)", sortSmall);

    auto sortMedium = [&] {
        QueryResult r = engine.select(medium, std::span<const FilterPredicate>{}, &sort);
        doNotOptimize(r);
    };
    BENCH_SOLO_CUSTOM("qe select sorted(100)", sortMedium);

    auto sortLarge = [&] {
        QueryResult r = engine.select(large, std::span<const FilterPredicate>{}, &sort);
        doNotOptimize(r);
    };
    BENCH_SOLO_CUSTOM("qe select sorted(1000)", sortLarge);
}
