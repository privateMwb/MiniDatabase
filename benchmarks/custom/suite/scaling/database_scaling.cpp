...
static void bench_create_10() {
    ...
    BENCH_SOLO("db createTable(10)", create);
}
...
static void bench_create_100() {
    ...
    BENCH_SOLO("db createTable(100)", create);
}
...
static void bench_create_200() {
    ...
    BENCH_SOLO("db createTable(200)", create);
}
...
static void bench_get_at_scale() {
    ...
    BENCH_SOLO("db getTable(10)", getSmall);

    auto getMedium = [&] {
        Table* t = medium.getTable("t50");
        doNotOptimize(t);
    };
    BENCH_SOLO("db getTable(100)", getMedium);

    auto getLarge = [&] {
        Table* t = large.getTable("t150");
        doNotOptimize(t);
    };
    BENCH_SOLO("db getTable(200)", getLarge);
}
