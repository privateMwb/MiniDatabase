...
Drop the trailing number and just give the letter (e.g. `a`) to run
every benchmark in that group at once, instead of picking one:

```
./build/google_regression v1.2.0 a
```

If either side doesn't have the requested benchmark (or, for a letter
group, no benchmarks in it), the tool reports which one and exits
