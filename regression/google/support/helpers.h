...
// Keeps only the entries whose name is one of `names`, preserving
// relative order -- the multi-benchmark generalization of
// filterByName() above, used to restrict a comparison to a whole
// [LetterN] group's worth of benchmarks at once (e.g.
// `google_regression A`) rather than a single named benchmark.
inline std::vector<BenchmarkResult> filterByNames(const std::vector<BenchmarkResult>& results,
...
// for a whole [LetterN] group's worth of benchmarks (as returned by
// resolveLetterGroup()) rather than a single benchmark name -- used
// for `google_regression A`-style runs. Still reports (by throwing)
// if either side ends up with nothing after filtering, though in
// practice this only happens if a baseline's benchmark list changed
// between the two sides being compared, since both sides start from
// the same group.
inline void applyMethodFilter(std::vector<BenchmarkResult>& baselineResults,
                              std::vector<BenchmarkResult>& currentResults,
                              const std::vector<std::string>& names,
                              const std::string& currentLabel, const std::string& baselineLabel) {
    baselineResults = filterByNames(baselineResults, names);
    currentResults = filterByNames(currentResults, names);

    if (baselineResults.empty())
        throw std::runtime_error(baselineLabel + " has no benchmarks in that group.");

    if (currentResults.empty())
        throw std::runtime_error(currentLabel + " has no benchmarks in that group.");
}
...
// True if a benchmark filter argument is a bare group letter (e.g.
// "A", "a") -- exactly one alphabetic character and nothing else,
// matching one of the letter headers printed by printMethods() (e.g.
// "A (3)"). Distinct from looksLikeMethodId() above, which
// additionally requires trailing digits picking one member out of the
// group; this is the "run every benchmark in the group" form of the
// same scheme, so it's deliberately checked separately rather than
// folded into that function.
...
// snapshot's own letter-grouped benchmark-name list -- the same
// grouping printMethods() itself builds -- and returns every
// benchmark name in that group, in the same [A1], [A2], ... order
// printMethods() would list them. Case-insensitive. Throws if the
// letter has no group; only call this once looksLikeLetterGroup() has
