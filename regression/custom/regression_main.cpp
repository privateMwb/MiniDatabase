...
void printUsage() {
    ...
    std::cerr << "\n"
              ...
              << RESET << DARK_GRAY << " (e.g. \"u1\"), or just the letter (e.g. \"u\") to run\n"
              << "every method in that group" << RESET << "\n";
}
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        // u1` works the same way `tests U1` runs a suite by id. A bare
        // letter with no trailing digit (e.g. "u") is the "whole
        // group" form of the same shortcut -- resolved to every
        // method name under that letter instead of just one.
        std::vector<std::string> methodGroup;

        if (!methodFilter.empty() && looksLikeMethodId(methodFilter)) {
            methodFilter = resolveMethodId(methodFilter, baselineFile);
        } else if (!methodFilter.empty() && looksLikeLetterGroup(methodFilter)) {
            methodGroup = resolveLetterGroup(methodFilter, baselineFile);
        }
        ...
        if (!methodGroup.empty()) {
            applyMethodFilter(baselineResults, currentResults, methodGroup, currentColumnLabel,
                              baselineColumnLabel);
        } else if (!methodFilter.empty()) {
