...
// Keeps only the entries whose operation is one of `operations`,
// preserving relative order -- the multi-method generalization of
// filterByOperation() above, used to restrict a comparison to a whole
// [LetterN] group's worth of methods at once (e.g. `regression A`)
// rather than a single named method.
inline std::vector<BenchmarkResult> filterByOperations(const std::vector<BenchmarkResult>& results,
...
// Group-filter overload of applyMethodFilter() above: same idea, but
// for a whole [LetterN] group's worth of methods (as returned by
// resolveLetterGroup()) rather than a single method name -- used for
// `regression A`-style runs. Still reports (by throwing) if either
// side ends up with nothing after filtering, though in practice this
// only happens if a baseline's method list changed between the two
// sides being compared, since both sides start from the same group.
inline void applyMethodFilter(std::vector<BenchmarkResult>& baselineResults,
                              std::vector<BenchmarkResult>& currentResults,
                              const std::vector<std::string>& methods,
                              const std::string& currentLabel, const std::string& baselineLabel) {
    baselineResults = filterByOperations(baselineResults, methods);
    currentResults = filterByOperations(currentResults, methods);

    if (baselineResults.empty())
        throw std::runtime_error(baselineLabel + " has no benchmark methods in that group.");

    if (currentResults.empty())
        throw std::runtime_error(currentLabel + " has no benchmark methods in that group.");
}
...
// True if a method filter argument is a bare group letter (e.g. "A",
// "a") -- exactly one alphabetic character and nothing else, matching
// one of the letter headers printed by printMethods() (e.g. "A (3)").
// Distinct from looksLikeMethodId() above, which additionally requires
// trailing digits picking one member out of the group; this is the
// "run every method in the group" form of the same scheme, so it's
// deliberately checked separately rather than folded into that
// function.
inline bool looksLikeLetterGroup(const std::string& s) {
    return s.size() == 1 && std::isalpha(static_cast<unsigned char>(s.front()));
}

// Resolves a bare group letter (e.g. "A") against one baseline
// snapshot's own letter-grouped method list -- the same grouping
// printMethods() itself builds -- and returns every method name in
// that group, in the same [A1], [A2], ... order printMethods() would
// list them. Case-insensitive. Throws if the letter has no group;
// only call this once looksLikeLetterGroup() has already confirmed
// the argument's shape.
inline std::vector<std::string> resolveLetterGroup(const std::string& letterArg,
                                                   const std::string& file) {
    char letter = static_cast<char>(std::toupper(static_cast<unsigned char>(letterArg.front())));
    ...
    throw std::runtime_error("Unknown method group: '" + letterArg + "'.");
}
