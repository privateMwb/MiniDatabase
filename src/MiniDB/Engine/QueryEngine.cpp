...
// ============================================================
QueryResult QueryEngine::select(Table& table, std::span<const FilterPredicate> predicates,
                                const SortCondition* sort, std::size_t limit) {
    QueryResult result;
...
//  Section 3 — Aggregate
// ============================================================
std::size_t QueryEngine::count(Table& table, std::span<const FilterPredicate> predicates) {
    std::size_t total = 0;
    ArenaScope scope(arena);
...
double QueryEngine::sum(Table& table, const FieldName& field,
...
double QueryEngine::avg(Table& table, const FieldName& field,
                        std::span<const FilterPredicate> predicates) {
    double total = 0.0;
    std::size_t count = 0;
    ArenaScope scope(arena);
...
double QueryEngine::max(Table& table, const FieldName& field,
                        std::span<const FilterPredicate> predicates) {
    double result = 0.0;
    bool found = false;
    ArenaScope scope(arena);
...
double QueryEngine::min(Table& table, const FieldName& field,
                        std::span<const FilterPredicate> predicates) {
    double result = 0.0;
    bool found = false;
    ArenaScope scope(arena);
...
bool QueryEngine::evaluatePredicate(const Record& record, const FilterPredicate& predicate) const {
    if (!record.hasField(predicate.field))
        return false;
...
bool QueryEngine::evaluateAll(const Record& record,
    ...
    for (const FilterPredicate& p : predicates) {
        if (!evaluatePredicate(record, p))
            return false;
    }
    return true;
}

void QueryEngine::sortResults(Vector<Record>& records, const SortCondition& sort) const {
    ...
    std::sort(records.begin(), records.end(), [this, &sort](const Record& a, const Record& b) {
        const Json& fa = a.getFieldRef(sort.field);
        const Json& fb = b.getFieldRef(sort.field);
        return sort.order == SortOrder::ASC ? compareValues(fa, fb, Op::LT)
                                            : compareValues(fb, fa, Op::LT);
    });
}

bool QueryEngine::compareValues(const Json& a, const Json& b, Op op) const {
