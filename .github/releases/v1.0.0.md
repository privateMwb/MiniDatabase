...
The first stable release of **MiniDatabase**, an embedded C++ database engine built on this project's own allocator, container, concurrency, and JSON libraries.
...
MiniDatabase has no natural drop-in standard-library
...
(`libs/internal/`), focusing on MiniDatabase's own implementation.
...
This release represents the first stable version of MiniDatabase and establishes its initial storage engine, query engine, concurrency model, and cross-platform build and packaging pipeline.

## Installation

Clone the repository and integrate MiniDatabase into your C++ project using the provided CMake configuration.
