...
static void insert_update_delete_multipage() {
    Database db("app");
...
static void drop_recreate_no_disturb() {
    Database db("app");
    CHK(db.createTable("users", usersSchema()) == Status::OK);
...
static void compact_preserves_live() {
    Database db("app");
    CHK(db.createTable("users", usersSchema()) == Status::OK);
    Table* users = db.getTable("users");
...
// round trip intact.
static void full_session_save_load() {
...
static void run_tests() {
    RUN(insert_update_delete_multipage);
    RUN(drop_recreate_no_disturb);
    RUN(compact_preserves_live);
    RUN(full_session_save_load);
}
