...
- Every test is a free `static void` function, registered with `RUN(...)`
  ...
  bottom — one test file, one suite, no shared driver across files.
...
- Compile-time contract checks (`move_only.cpp`,
  `table_copy_assign_signature.cpp`) use `static_assert` directly inside
  or alongside the test function — a regression there fails to
  **compile**, not just fails a runtime check.
- Fixture helpers (schemas, temp paths, record builders) live in an
  anonymous namespace above the test functions in the same file, not
  shared across files.
- Files that touch disk build their own path under
  `std::filesystem::temp_directory_path()` and remove it at the end of
  each test that created it.
