...
static void page_parse_error() {
    Page p;
    CHK(p.deserialize("not json at all") == Status::PARSE_ERROR);
}

static void table_parse_error() {
...
// bypasses the cache).
static void evict_dirty_persists() {
...
static void evict_uncached_idempotent() {
...
static void flush_uncached_not_found() {
...
static void write_oversized_oom() {
...
// permanent.
static void fetch_loads_on_miss() {
...
static void run_tests() {
    RUN(record_parse_error);
    RUN(page_parse_error);
    RUN(table_parse_error);
    RUN(evict_dirty_persists);
    RUN(evict_uncached_idempotent);
    RUN(flush_uncached_not_found);
    RUN(flush_clean_noop);
    RUN(write_oversized_oom);
    RUN(fetch_loads_on_miss);
}
