...
static void drop_table_removes() {
    Database db("app");
    CHK(db.createTable("users", makeSchema()) == Status::OK);
...
static void drop_missing_fails() {
    Database db("app");
...
static void get_missing_nullptr() {
    Database db("app");
...
static void table_id_no_collision() {
    Database db("app");
...
// missing.
static void load_corrupt_untouched() {
...
// than crashing.
static void load_missing_error() {
    Database db("app");
...
static void run_tests() {
    ...
    RUN(create_dup_fails);
    RUN(drop_table_removes);
    RUN(drop_missing_fails);
    RUN(get_missing_nullptr);
    RUN(table_id_no_collision);
    RUN(json_round_trip);
    RUN(save_load_round_trip);
    RUN(load_corrupt_untouched);
    RUN(load_missing_error);
