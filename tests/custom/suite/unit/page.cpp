...
// empty, clean state.
static void default_ctor_invalid_id() {
    Page p;
...
static void explicit_ctor_empty_state() {
    Page p(42);
    CHK(p.getID() == 42);
    CHK(p.isEmpty());
    CHK(p.recordCount() == 0);
...
static void add_record_marks_dirty() {
    Page p(1);
...
static void add_record_fills_rejects() {
    Page p(1);
...
static void get_record_matching() {
    Page p(1);
...
static void get_record_missing_null() {
    Page p(1);
...
static void get_record_at_incl_deleted() {
    Page p(1);
    CHK(p.addRecord(Record(1)) == Status::OK);
    CHK(p.deleteRecord(1) == Status::OK);
...
static void update_existing_replaces() {
    Page p(1);
    Record original(1);
...
static void update_missing_not_found() {
    Page p(1);
...
static void update_deleted_not_found() {
    Page p(1);
    CHK(p.addRecord(Record(1)) == Status::OK);
    CHK(p.deleteRecord(1) == Status::OK);
...
// compact() runs.
static void delete_marks_deleted_dirty() {
    Page p(1);
    CHK(p.addRecord(Record(1)) == Status::OK);
    CHK(p.deleteRecord(1) == Status::OK);
...
static void delete_missing_not_found() {
    Page p(1);
...
static void compact_removes_deleted() {
    Page p(1);
    CHK(p.addRecord(Record(1)) == Status::OK);
...
static void compact_frees_slots() {
    Page p(1);
    for (RecordID i = 0; i < DBConstants::MAX_RECORDS_PAGE; ++i) {
        CHK(p.addRecord(Record(i)) == Status::OK);
    }
...
static void to_json_excludes_deleted() {
    Page p(1);
    CHK(p.addRecord(Record(10)) == Status::OK);
...
static void json_round_trip_live() {
    Page original(4);
    Record r1(1);
...
static void deserialize_malformed() {
    Page p;
...
static void move_ctor_transfers_state() {
    Page original(5);
...
static void run_tests() {
    RUN(default_ctor_invalid_id);
    RUN(explicit_ctor_empty_state);
    RUN(add_record_marks_dirty);
    RUN(add_record_fills_rejects);
    RUN(get_record_matching);
    RUN(get_record_missing_null);
    RUN(get_record_skips_deleted);
    RUN(get_record_at_by_index);
    RUN(get_record_at_incl_deleted);
    RUN(update_existing_replaces);
    RUN(update_missing_not_found);
    RUN(update_deleted_not_found);
    RUN(delete_marks_deleted_dirty);
    RUN(delete_missing_not_found);
    RUN(compact_removes_deleted);
    RUN(compact_frees_slots);
    RUN(to_json_excludes_deleted);
    RUN(json_round_trip_live);
    RUN(serialize_round_trip);
    RUN(deserialize_malformed);
    RUN(move_ctor_transfers_state);
