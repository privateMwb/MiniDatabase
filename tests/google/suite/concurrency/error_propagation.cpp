...
TEST(ConcurrencyErrorPropagation, AllSucceedReturnsOk) {
    ...
    EXPECT_EQ(conc.loadAllTablesParallel(db, dir + "/db"), Status::OK);
}
...
TEST(ConcurrencyErrorPropagation, LastNonOkStatusWins) {
    const std::string dir = tempDir("error_last_wins");
    const std::string base = dir + "/db";

    Database db("shop");
    ASSERT_EQ(db.createTable("a", Vector<ColumnDef>{}), Status::OK);
    ASSERT_EQ(db.createTable("b", Vector<ColumnDef>{ColumnDef{"n", ColumnType::INT, false}}),
              Status::OK);
    ASSERT_EQ(db.createTable("c", Vector<ColumnDef>{}), Status::OK);
    ...
    EXPECT_EQ(result, Status::INVALID_TYPE);
    EXPECT_NE(result, Status::IO_ERROR);
}
...
TEST(ConcurrencyErrorPropagation, FailurePartialApply) {
    ...
    EXPECT_EQ(db.getTable("b")->recordCount(), 0); // b's insert never applied
    EXPECT_EQ(db.getTable("c")->recordCount(), 3); // c succeeded independently
}
...
// LastNonOkStatusWins this can't distinguish "last" from "first" -- it
// only confirms the fold and partial-success behavior generalize to save.
TEST(ConcurrencyErrorPropagation, SaveSharesFold) {
    ...
    EXPECT_TRUE(
        std::filesystem::exists(base + "_c.json")); // c's success still applied despite a/b failing
}
