...
TEST(StatusHandling, RecordDeserializeMalformedReturnsParseError) {
    Record r;
    EXPECT_EQ(r.deserialize("not json at all"), Status::PARSE_ERROR);
}

TEST(StatusHandling, PageDeserializeMalformedReturnsParseError) {
    Page p;
    EXPECT_EQ(p.deserialize("not json at all"), Status::PARSE_ERROR);
}

TEST(StatusHandling, TableDeserializeMalformedReturnsParseError) {
    Table t("", DBConstants::INVALID_TABLE_ID, Vector<ColumnDef>{});
    EXPECT_EQ(t.deserialize("not json at all"), Status::PARSE_ERROR);
}
...
// bypasses the cache).
TEST(StatusHandling, EvictDirtyPagePersistsBeforeRemoval) {
    StorageEngine engine(tempDir("evict_dirty"));

    Page p(0);
    Record r(1);
    (void)r.setField("name", Json("Ada"));
    ASSERT_EQ(p.addRecord(r), Status::OK);
    ASSERT_TRUE(p.isDirty());

    engine.cachePage("users", std::move(p));
    ASSERT_TRUE(engine.isCached("users", 0));

    ASSERT_EQ(engine.evictPage("users", 0), Status::OK);
    EXPECT_FALSE(engine.isCached("users", 0));
    ...
    Page fromDisk;
    ASSERT_EQ(engine.readPageFromDisk("users", 0, fromDisk), Status::OK);
    ASSERT_EQ(fromDisk.recordCount(), 1);
...
TEST(StatusHandling, EvictUncachedPageIsIdempotentOk) {
    StorageEngine engine(tempDir("evict_uncached"));
    EXPECT_EQ(engine.evictPage("users", 42), Status::OK);
}
...
TEST(StatusHandling, FlushUncachedPageReturnsNotFound) {
    StorageEngine engine(tempDir("flush_uncached"));
    EXPECT_EQ(engine.flushPage("users", 42), Status::NOT_FOUND);
}

// Verifies flushPage on a clean (non-dirty) cached page is a no-op that
// still reports Status::OK.
TEST(StatusHandling, FlushCleanPageIsOkNoop) {
    StorageEngine engine(tempDir("flush_clean"));

    Page p(0); // never modified: not dirty
    ASSERT_FALSE(p.isDirty());
    engine.cachePage("users", std::move(p));

    EXPECT_EQ(engine.flushPage("users", 0), Status::OK);
}
...
TEST(StatusHandling, WritepageOversizedReturnsOutOfMemory) {
    ...
    EXPECT_EQ(engine.writePage("users", p), Status::OUT_OF_MEMORY);
}
...
// permanent.
TEST(StatusHandling, FetchpageLoadsFromDiskOnCacheMiss) {
    ...
    ASSERT_FALSE(reader.isCached("users", 0)); // fresh instance, empty cache

    Page* fetched = reader.fetchPage("users", 0);
    ASSERT_NE(fetched, nullptr);
    EXPECT_EQ(fetched->recordCount(), 1);
}
