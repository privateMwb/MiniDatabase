...
// COMPILE, not just fails a runtime check.
TEST(TableCopyAssignSignature, TableCopyAssignDeleted) {
    ...
    SUCCEED(); // the static_asserts above are the real assertions
}

// Verifies move assignment -- the only assignment path Table has left --
// actually transfers state correctly.
TEST(TableCopyAssignSignature, TableMoveAssignTransfersState) {
    Table source("orders", 1, Vector<ColumnDef>{});
    ASSERT_EQ(source.insertRecord(Record(1)), Status::OK);

    Table target("empty", 2, Vector<ColumnDef>{});
    target = std::move(source);

    EXPECT_EQ(target.getName(), "orders");
    EXPECT_EQ(target.recordCount(), 1);

    // NOLINTNEXTLINE(clang-analyzer-cplusplus.Move)
    EXPECT_EQ(source.getID(), DBConstants::INVALID_TABLE_ID);
}
