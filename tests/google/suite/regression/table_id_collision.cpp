...
// pre-fix behavior).
TEST(TableIdCollision, ReloadNewPageIdUnique) {
    Table t = loadTableWithOneFullPage(100);
    ASSERT_EQ(t.pageCount(), 1);

    ASSERT_EQ(t.insertRecord(Record(9999)), Status::OK);
...
TEST(TableIdCollision, ReloadMultiplePagesUniqueIds) {
    ...
    for (int fillCount = 0; fillCount < 2; ++fillCount) {
        for (std::uint32_t i = 0; i < DBConstants::MAX_RECORDS_PAGE; ++i) {
            ASSERT_EQ(t.insertRecord(Record(nextRecordId++)), Status::OK);
        }
    }
    ASSERT_EQ(t.pageCount(), 3);
